...
function createOutputBoxes(generated_outputs, highlight_setup_id) { // custom-factgenie ONELINE
...
function fetchExample(dataset, split, example_idx) {
    ...
    $.get(`${url_prefix}/example`, {
        ...
        createOutputBoxes(data.generated_outputs, window.highlight_setup_id); // custom-factgenie ONELINE
...
function showSelectedCampaigns() {
    ...
    if (selected_campaigns.length == 0) { // custom-factgenie ONELINE
...
$("#badgesSwitch").on("change", function () {
    createOutputBoxes(window.generated_outputs, window.highlight_setup_id); // custom-factgenie ONELINE
    showSelectedCampaigns();
    updateDisplayedAnnotations();
});
