...
# custom-factgenie START
GCS_BUCKET_NAME = os.environ.get('GCS_USER_DATA_BUCKET')
...
# custom-factgenie START
@app.route('/campaign_exists/<campaign_id>')
...
# @login_required custom-factgenie ONELINE
def llm_campaign_create():
    ...
    campaign_data = data.get("campaignData")
    config = data.get("config")
    setup_id = config.get("setup_id") # custom-factgenie ONELINE
    ...
    try:
        llm_campaign.create_llm_campaign(app, mode, campaign_id, config, campaign_data, datasets, setup_id) # custom-factgenie ONELINE
        workflows.load_campaign(app, campaign_id=campaign_id)
    except Exception as e:
        traceback.print_exc()
        return utils.error(f"Error while creating campaign: {e}")
...
def submit_annotations():
    ...
    logger.info(f"Received annotations for {campaign_id} by {annotator_id} and {user_id}" )

    try:
        if not user_id.endswith('/'):
            user_id += '/'
        blob = bucket.blob("users/"+user_id+  data['user_id']+"_response_0.jsonl")

        json_data_string = json.dumps(data) + '\n'
        blob.upload_from_string(json_data_string, content_type='application/jsonl')
        print(f"Successfully added user response data '{user_id}' in bucket.")

        return jsonify({
            'success': True,
            'message': 'User reponse data saved successfully'
        })
    except Exception as e:
        return jsonify({
            'success': False,
...
def save_annotations():
    ...
    try:
        ...
        user_folder_prefix = "users/"+user_id+  data['user_id']+"_response_"
        
        # List all blobs that start with the user's folder prefix.
        # This is efficient as it only lists blobs within that "directory".
        blobs_in_user_folder = list(bucket.list_blobs(prefix=user_folder_prefix))

        progress_number = len(blobs_in_user_folder)
        while progress_number>0:
            current_file = "users/"+user_id+  data['user_id']+"_response_{}.jsonl".format(progress_number)
            if(current_file in str(blobs_in_user_folder)):
                break
            progress_number=progress_number-1
        progress_number=progress_number+1
       

        blob = bucket.blob(("users/"+user_id+  data['user_id']+"_response_{}.jsonl").format(progress_number))       
        json_data_string = json.dumps(data) + '\n'
    
       
        blob.upload_from_string(json_data_string, content_type='application/jsonl')
        print(f"Successfully added user progress data '{user_id}' in bucket.")
...
def save_user_data():
    try:
        ...
        if not campaign_id.endswith('/'):
            campaign_id += '/'

       

        blob = bucket.blob("users/"+campaign_id+ data.get('campaign_id')+"_generation_0.jsonl")
        if blob.exists():
            print(f"Folder '{campaign_id}'  exists in bucket.")

        
        else:
            return jsonify({
            'success': False,
            'error': "User id not found in bucket"
            }), 500
        
        now = datetime.now()
        current_time_str = now.strftime("%y_%m_%d_%H_%M_%S")
        blob1 = bucket.blob("login_attempts/"+data.get('campaign_id')+"_"+current_time_str)
        json_data_string = json.dumps(data) + '\n'
    
       
        blob1.upload_from_string(json_data_string, content_type='application/jsonl')
        print(f"Successfully added user login data '{campaign_id}' in bucket.")
...
def fetch_user_data():
    try:
        ...
        user_folder_prefix = "users/"+campaign_id+  data.get('campaign_id')+"_response_"
        blobs_in_user_folder = list(bucket.list_blobs(prefix=user_folder_prefix))
        if(len(blobs_in_user_folder)!= 0):
            progress_number = len(blobs_in_user_folder)
            current_file = "users/"+campaign_id+ data.get('campaign_id')+"_response_{}.jsonl".format(0)
            if(current_file not in str(blobs_in_user_folder)):
                
                while progress_number>0:
                    current_file = "users/"+campaign_id+ data.get('campaign_id')+"_response_{}.jsonl".format(progress_number)
                    if(current_file in str(blobs_in_user_folder)):
                        break
                    progress_number=progress_number-1
            else:
        ...
        if blob.exists():
            # blob=blob2
            print(f"Verified Folder, '{campaign_id}'  exists in bucket.")
        else:
            return jsonify({
            'success': False,
            'error': "User id not found in bucket"
        ...
        json_objects = []
        # Download the entire content of the JSONL file as a single string
        jsonl_content = blob.download_as_text()
        print(f"Downloaded {len(jsonl_content)} characters from bucket.")
        with io.StringIO(jsonl_content) as f:
            reader = jsonlines.Reader(f)
            for obj in reader:
                json_objects.append(obj)
    
        print(f"Successfully loaded {len(json_objects)} JSON objects from blob.")

        if(flag==1):
            json_objects = json_objects[0]['annotation_set']
